...
// instruction decode and execution
    ...
    //printf("opcode = %x, eip = %x, esp = %x, (esp) = %x, ebp = %x, (ebp+0xc) = %x, edx = %x, eax = %x, src.val = %x,dest.val = %x\n",opcode,cpu.eip,cpu.esp,vaddr_read(cpu.esp, SREG_CS, 4),cpu.ebp,vaddr_read(cpu.ebp+0xc, SREG_CS, 4),cpu.edx,cpu.eax,opr_src.val,opr_dest.val);
    //printf("esp = %#08x\n",cpu.esp);
#else
