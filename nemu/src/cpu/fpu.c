...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	if (!overflow)
	{
		...
		if(grs < 4)sig_grs = sig_grs >> 3 ;
		else if(grs == 4){
		    sig_grs = sig_grs>>3;
		    if((sig_grs & 0x1) == 1)sig_grs++;   //变为偶数
		}
		else{
		    sig_grs = sig_grs>>3;
		    sig_grs++;
...
uint32_t internal_float_mul(uint32_t b, uint32_t a)
{
	...
	uint32_t my_result = internal_normalize(f.sign, exp_res, sig_res);
	
	//test
	/*
	FLOAT test_fa,test_fb,test_f;
	test_fa.val = a;
	test_fb.val = b;
	test_f.fval = test_fa.fval*test_fb.fval;
	uint32_t tst_result = test_f.val;
	FLOAT my_f;
	my_f.val = my_result;
	
	if(my_result!=tst_result)
	    printf("a = %#x,b = %#x, my_result = %#x, test_result = %#x,my_exponent = %#x,test_exponent = %#x\n",a,b,my_result,tst_result,my_f.exponent,test_f.exponent);
	*/
	return my_result;
}
